        ...
        <div className="d-flex align-items-center gap-3 difficulty-container">
            ...
            <div className="small">
                <div className="d-flex align-items-center gap-2 mb-1">
                    ...
                    <span className="opacity-50">Easy:</span> <span className="fw-bold">{stats.Beginner}</span>
                </div>
                <div className="d-flex align-items-center gap-2 mb-1">
        ...
        <div className="d-flex vh-100 overflow-hidden text-white w-100 main-app-layout" style={{ background: '#020617', fontFamily: "'Inter', sans-serif" }}>
            ...
            <style>{`
                ...
                @media (max-width: 768px) {
                    .challenge-text { font-size: 1.25rem; }
                    .difficulty-container { flex-direction: column; align-items: flex-start !important; }
...
const DashboardView = ({ user, setTab, history }) => {
    ...
    const stats = useMemo(() => {
        ...
        const avgAccuracy = totalQs > 0 ? Math.round((totalActualScore / (totalQs * 10)) * 100) : 0;
        
        const sessionMap = new Map();
        history.forEach(h => {
            if (!sessionMap.has(h.sessionId)) {
                sessionMap.set(h.sessionId, h.difficulty || "Beginner");
            }
        });

        const diffCounts = { Beginner: 0, Intermediate: 0, Advanced: 0, total: sessionMap.size };
        sessionMap.forEach((difficulty) => {
            if (diffCounts[difficulty] !== undefined) diffCounts[difficulty]++;
        });
        ...
        return { 
            ...
            totalSolved: totalQs, 
            rating: avgAccuracy > 70 ? "A" : avgAccuracy >= 40 ? "B" : "C", 
            level, 
            diffCounts 
        };
    }, [history]);
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div className="row g-3 g-md-4 mb-5">
                ...
                <div className="col-12 col-md-3">
                    <div className="p-3 p-md-4 h-100 d-flex align-items-center justify-content-center" style={glassStyle}>
                        <DifficultyCircle stats={stats.diffCounts} />
                    </div>
                </div>
            </div>
            ...
            <div className="row g-4">
                <div className="col-lg-8">
                    <div className="p-4 p-md-5 rounded-5 h-100" style={{ minHeight: '300px', background: 'linear-gradient(135deg, rgba(59,130,246,0.15), transparent)', border: '1px solid rgba(59,130,246,0.2)' }}>
                ...
                <div className="col-lg-4">
                    <div className="p-4 rounded-5" style={{ ...glassStyle, minHeight: '300px' }}>
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            <div className="p-3 p-md-5 mb-4" style={glassStyle}>
                {/* Row 1: Knowledge Domains - Set to col-12 to span full width */}
                ...
                {/* Row 2: Parameters - Balanced to occupy all 12 columns (5 + 5 + 2 = 12) */}
                <div className="row g-4 align-items-end">
                    ...
                    <div className="col-12 col-md-2 text-center">
                        ...
                        <div className={`timer-btn-toggle ${isTimed ? 'active' : ''}`} onClick={() => setIsTimed(!isTimed)}>
                            {isTimed ? <Timer size={22}/> : <TimerOff size={22}/>}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="row g-3 g-md-4">
                {[
                    ...
                    <div className="col-12 col-md-4" key={p.id}>
                        <div className="protocol-card p-4 p-md-5 h-100" style={glassStyle} onClick={() => handleStart(p.id)}>
    ...
    const submitBatch = useCallback(async (finalAnswers) => {
        ...
        try {
            const res = await fetch(`${API_URL}/evaluate-batch`, {
                method: "POST", headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ username: user.name, answers: finalAnswers, domains, sessionId, type, difficulty })
            });
            const data = await res.json();
            setResults(data.results);
        } catch (e) { alert("Evaluation error."); }
        finally { setEvaluating(false); }
    }, [user.name, domains, sessionId, type, difficulty]);
        ...
        <div className="mx-auto" style={{ maxWidth: '850px' }}>
            <div className="p-3 p-md-5" style={glassStyle}>
                ...
                {evaluating ? (
                        ...
                        <div className="mb-5">{results.map((res, i) => (
                            <div key={i} className="p-4 mb-3 rounded-4" style={{background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)'}}>
                                <div className="d-flex flex-column flex-md-row justify-content-between mb-2 small fw-bold">
                                    ...
                                    <span className={res.score >= 8 ? 'text-success' : 'text-danger'}>{res.score * 10}% Accuracy</span>
                                </div>
                    ...
                    <>
                        <div className="mb-5 h2 challenge-text">
                            {typeof currentChallenge === 'string' && currentChallenge.includes('Question:') ? (
                                currentChallenge.split('Question:').map((text, i) => <div key={i} className={i === 0 ? "scenario-text opacity-50 fs-5 mb-2" : ""}>{text}</div>)
                            ) : currentChallenge}
                        </div>
                                ...
                                <div key={s.id} className="p-3 p-md-4 mb-3 rounded-4" style={glassStyle}>
                                    <div className="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center mb-4 gap-3">
                                        <div onClick={() => setExpandedSession(expandedSession === s.id ? null : s.id)} style={{ cursor: 'pointer' }}>
                                            <h2 className="fw-black mb-0 tracking-tight text-uppercase" style={{ fontSize: '1.4rem' }}>
                                                {s.domain || "General"}
                                            </h2>
                                        ...
                                        <div className="px-3 py-2 rounded-4 d-flex flex-wrap align-items-center gap-2 gap-md-3" style={metaBoxStyle}>
                                            ...
                                            <div className="d-flex align-items-center gap-2">
                                                ...
                                                <span className="small fw-bold opacity-80">{s.items.length} Qs</span>
                                            </div>
                                            <div className="d-none d-md-block" style={{ width: '1px', height: '12px', background: 'rgba(255,255,255,0.2)' }}></div>
                                            <div className="px-2 rounded-pill" style={{ background: diffTheme.bg, border: `1px solid ${diffTheme.color}33` }}>
                                                <span className="fw-black uppercase tracking-tighter" style={{ fontSize: '0.65rem', color: diffTheme.color }}>
                                                    {s.difficulty}
                                                </span>
                                            </div>
                                        </div>
                                    ...
                                    <div className="d-flex align-items-center gap-4">
                                        ...
                                        <button className="btn btn-primary btn-sm rounded-pill px-3 px-md-4 fw-bold" style={{ fontSize: '0.65rem', letterSpacing: '1px' }} onClick={() => setExpandedSession(expandedSession === s.id ? null : s.id)}>
                                            {expandedSession === s.id ? "HIDE" : "VIEW"}
                                        </button>
                                    </div>
                                                    ...
                                                    {s.items.map((item, idx) => {
                                                        ...
                                                        return (
                                                            <div key={idx} className="col-12">
                                                                <div className="p-3 rounded-4" style={{ background: 'rgba(255,255,255,0.03)', borderLeft: `4px solid ${item.score >= 8 ? '#10b981' : (item.score < 5 ? '#ef4444' : '#3b82f6')}` }}>
                                                                    <div className="d-flex justify-content-between align-items-center mb-2">
                                                                        ...
                                                                        <span className={`fw-black ${item.score >= 8 ? 'text-success' : (item.score < 5 ? 'text-danger' : 'text-primary')}`}>
                                                                            {isObjective ? (item.score >= 8 ? 'CORRECT' : 'INCORRECT') : `${itemPercent}%`}
                                                                        </span>
                                                                    </div>
